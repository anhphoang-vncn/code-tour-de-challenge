<template>
  <div class="view">
    <Panel title="Danh sách cuộc thi">
      <div slot="header">
        <el-input
          v-model="keyword"
      ...
      <el-table
        ...
        <el-table-column
          ...
          <div slot-scope="scope">
            <icon-btn name="Chỉnh sửa" icon="edit" @click.native="goEdit(scope.row.id)"></icon-btn>
            <icon-btn name="Bài tập" icon="list-ol" @click.native="goContestProblemList(scope.row.id)"></icon-btn>
            <icon-btn name="Thông báo" icon="info-circle"
                      @click.native="goContestAnnouncement(scope.row.id)"></icon-btn>
            <icon-btn icon="download" name="Tải xuống bài nộp được chấp nhận"
