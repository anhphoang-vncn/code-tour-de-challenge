  ...
  <el-row type="flex" :gutter="20">
      ...
      <panel :title="$t('m.System_Overview')" v-if="isSuperAdmin">
        ...
        <p>{{$t('m.HTTPS_Status')}}:
          <el-tag :type="https ? 'success' : 'danger'" size="small">
            {{ https ? 'Đã bật' : 'Vô hiệu hóa'}}
          </el-tag>
        </p>
        <p>{{$t('m.Force_HTTPS')}}:
          <el-tag :type="forceHttps ? 'success' : 'danger'" size="small">
            {{forceHttps ? 'Đã bật' : 'Vô hiệu hóa'}}
          </el-tag>
        </p>
        <p>{{$t('m.CDN_HOST')}}:
    ...
    <el-col :md="14" :lg="16" v-if="isSuperAdmin">
      <div class="info-container">
        ...
        <info-card color="#67C23A" icon="el-icon-fa-list" message="Tổng bài nộp" class="info-item"
                   :value="infoData.today_submission_count"></info-card>
        <info-card color="#409EFF" icon="el-icon-fa-trophy" message="Cuộc thi" class="info-item"
                   :value="infoData.recent_contest_count"></info-card>
      </div>
      <panel style="margin-top: 5px">
        <span slot="title" v-loading="loadingReleases">Ghi chú bản cập nhật
        <el-popover placement="right" trigger="hover">
          <i slot="reference" class="el-icon-fa-question-circle import-user-icon"></i>
          <p>Vui lòng nâng cấp lên phiên bản mới nhất để tận hưởng các tính năng mới. </p>
        ...
        <el-collapse v-model="activeNames" v-for="(release, index) of releases" :key="'release' + index">
          <el-collapse-item :name="index+1">
            ...
            <p>Nội dung trong bản cập nhật này: </p>
