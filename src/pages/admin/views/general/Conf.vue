  ...
  <div class="view">
    <Panel :title="$t('m.SMTP_Config')">
      <el-form label-position="left" label-width="70px" :model="smtp">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item :label="$t('m.Server')" label-width="90px" required>
      ...
      <el-button type="primary" @click="saveSMTPConfig">Lưu</el-button>
      ...
      <el-form label-position="left" label-width="100px" ref="form" :model="websiteConfig">
        <el-row :gutter="20">
          ...
          <el-col :span="24">
            ...
            <el-col :span="6">
              <el-form-item :label="$t('m.Submission_List_Show_All')" label-width="230px">
                <el-switch
              ...
              <el-form-item v-for="(forum_sort, index) in websiteConfig.forum_sort" :key="$t('m.Sort') + index">
                <Accordion :title="$t('m.Sort') + (index + 1)" style="margin-left: -100px;">
                  ...
                  <el-row :gutter="20">
                    ...
                    <el-col :span="12">
                      <el-form-item :label="$t('m.Sort_Permission')" label-width="120px" required>
...
<script>
  ...
  export default {
    ...
    mounted () {
      api.getSMTPConfig().then(res => {
        if (res.data.data) {
          ...
          this.init = true
          this.$warning('Vui lòng thiết lập cấu hình SMTP lúc đầu')
        }
      })
    ...
    methods: {
      ...
      testSMTPConfig () {
        this.$prompt('Vui lòng nhập email của bạn', '', {
          inputPattern: /[\w!#$%&'*+/=?^_`{|}~-]+(?:\.[\w!#$%&'*+/=?^_`{|}~-]+)*@(?:[\w](?:[\w-]*[\w])?\.)+[\w](?:[\w-]*[\w])?/,
          inputErrorMessage: 'Lỗi định dạng email'
        }).then(({value}) => {
